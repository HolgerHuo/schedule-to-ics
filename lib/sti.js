        ...
        return {
            ...
            courses: day.courses.map(i => {
                if (i.type === 'custom') {
                    ...
                    return mapCourse(mappings[i], key);
                }
            })
        };
    });
 ...
 * @returns events
 */
const generateEvents = async (options, configUrl) => {
    ...
    const addEvent = (startTime, duration, course, repeat, note) => {
        ...
        const recurrence = repeat ? 'FREQ=DAILY;INTERVAL=' + repeat.schedulePeriod.toString() + ';COUNT=' + repeat.times.toSring() : null;
        ...
        if ([course.notes, note, classData.info.note, data.info.note].join('')) {
            event.description += '\n\n' + [course.notes, note, classData.info.note, data.info.note].join('\n').replace(/(^[ \t]*\n)/gm, "");
        }
    ...
    const iterateSchedule = (timeTable, schedule, fromDate, online, note) => {
        ...
        schedule.forEach(day => {
            ...
            day.courses.forEach((c, i) => {
                ...
                startTime.push(...timeTable.days[day['day']][classCount]);
    ...
    extras.forEach(i => {
        const fromDate = convertTime(moment.tz(i.fromDate, "MM-DD-YYYY", data.info.timezone).tz('UTC'));
        ...
        iterateSchedule(data.timeTable, scheduleMain, fromDate, i.onlineClass, i.note);
    }
    );
