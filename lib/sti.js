    ...
    const iterateSchedule = (timeTable, schedule, fromDate, online, note) => {
        ...
        schedule.forEach(day => {
            ...
            day.courses.forEach((c, i) => {
                ...
                startTime.push(timeTable.days[day['day']][classCount][0], timeTable.days[day['day']][classCount][1]);
                ...
                let addedDuration = timeTable.days[day['day']][classCount-1][2] || duration;

                if (options['skip-break'] !== "false") {
                    while (day.courses[classCount] === day.courses[classCount - 1] && !unmergeableBreaks.includes(classCount)) {
                        classCount = classCount + 1;
                        addedDuration = addDuration(addedDuration, timeTable.days[day['day']][classCount-1][2] || duration );
                        if (!course.skipBreak) {
